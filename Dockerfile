...
# So, pip install homeassistant will triggar some builds, requiring gcc. But gcc is removed from slim, so
...
RUN ["/bin/bash", "-c", "python3 -m venv . \
    ...
    && python3 -m pip install git+https://github.com/home-assistant/core.git@2021.5.5 "]
